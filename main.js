...
if (!state.creds.registered) {
  ...
  logEstado('⚡', 'Conectando de inmediato para evitar que WhatsApp cierre la sesión...', 'cyan');
}
...
function calcularBackoff(intento) {
  return Math.min(10 * 1000 * Math.pow(2, intento), 10 * 60 * 1000);
}
...
async function startSubBots() {
  const rutaJadiBot = join(__dirname, `./${global.jadi}`);
...
async function esperarConexionAbierta(timeoutMs = 15000) {
  const inicio = Date.now();
  while (!conn.ws || conn.ws.readyState !== ws.OPEN) {
    if (Date.now() - inicio > timeoutMs) {
      throw new Error('Tiempo de espera agotado: la conexión no llegó a abrirse');
    }
...
async function handleLogin() {
  ...
  if (metodoLogin === 'code' && numeroTelefono) {
    if (typeof conn.requestPairingCode === 'function') {
      try {
        logEstado('⏳', 'Esperando que la conexión esté lista...', 'yellow');
        await esperarConexionAbierta();
        logEstado('⏳', 'Generando código de emparejamiento...', 'yellow');
        let code = await conn.requestPairingCode(numeroTelefono);
        ...
        logEstado('💡', 'Reinicia el bot y vuelve a intentarlo (ya no debería tardar)~', 'yellow');
      }
    } else {
